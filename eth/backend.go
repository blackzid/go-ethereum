...
func New(ctx *node.ServiceContext, config *Config) (*Ethereum, error) {
	...
	if eth.protocolManager, err = NewHDCProtocolManager(eth.chainConfig, config.FastSync, config.NetworkId, eth.eventMux, eth.txPool, eth.pow, eth.blockchain, chainDb, eth.hdcValidators, config.PrivateKeyHex, eth, config.ExtraData, config.GasPrice, hdcDb); err != nil {
		return nil, err
	}
...
func (s *Ethereum) Start(srvr *p2p.Server) error {
	...
	glog.V(logger.Info).Infoln("end start")
	return nil
}
